...
- 起動した結果として対象リポジトリに変更があれば、`Git Context` として repository、branch 名、worktree、commit を返す
- 変更はあるがコミットがない場合は、`Git Context` に branch 名と worktree を含め、commit は `none` または省略でよい
